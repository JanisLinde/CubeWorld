...
////////////////////////////////////////////////////////////////////////////////
#include "../include/QuadTreeClass.h"
...
void QuadTreeClass::Shutdown()
{
	...
	// Release the vertex list in case it was not released earlier.
	if(m_vertexList)
	{
		delete [] m_vertexList;
		m_vertexList = 0;
...
void QuadTreeClass::CreateTreeNode(NodeType* node, float positionX, float positionZ, float width, ID3D11Device* device)
{
    ...
    result2 = device->CreateBuffer(&vertexBufferDesc, &vertexData, &node->vertexBuffer);
	if(FAILED(result2))
	{
	}
	...
	result2 = device->CreateBuffer(&indexBufferDesc, &indexData, &node->indexBuffer);
	if(FAILED(result2))
	{
	}
